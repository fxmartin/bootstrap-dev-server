...
if [[ -f "${SCRIPT_DIR}/lib/logging.sh" ]]; then
    ...
    init_logging "hcloud-provision"
else
    # Fallback if library not found (shouldn't happen in normal use)
    RED='\033[0;31m'
    GREEN='\033[0;32m'
    YELLOW='\033[1;33m'
    BLUE='\033[0;34m'
...
check_prerequisites() {
    ...
    if [[ -z "${DELETE_SERVER}" && -z "${RESCALE_SERVER}" && "${LIST_SERVERS}" != true ]]; then
        if [[ ! -f "${SSH_KEY_PATH}" ]]; then
            ...
            # shellcheck disable=SC2310  # Function in if condition is handled correctly
...
${port_line}    User ${SSH_USER}
    ...
    if [[ -f "${ssh_config}" ]]; then
        ...
        # shellcheck disable=SC2312  # Return value masking acceptable here
...
main() {
    ...
    log_phase "2: SSH Key Setup"
    upload_ssh_key

    log_phase "3: Server Creation"
    log_timer_start "server_creation"
    create_server
    wait_for_server
    log_timer_end "server_creation"

    log_phase "4: User Account Setup"
    setup_user_account
    update_ssh_config

    log_phase "5: Bootstrap Execution"
    log_timer_start "bootstrap_execution"
    run_bootstrap
    log_timer_end "bootstrap_execution"
